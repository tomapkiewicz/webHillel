...
class UserCreationFormWithEmail(UserCreationForm):
    email = forms.EmailField(required=True, label="", help_text="")
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError("El email ya está registrado, probá con otro.")

        username = self.cleaned_data.get("username")
        if username != email:
            raise forms.ValidationError("Los mails ingresados no son iguales")
        return email
...
class ProfileForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "experienciaComunitaria": forms.Textarea(
                attrs={
                    "class": "form-control mt-1",
                    "placeholder": "",
                    "required": True,
            ...
            "validado": forms.TextInput(
                attrs={"class": "form-control mt-3", "placeholder": "", "hidden": True}
            ),
        }
